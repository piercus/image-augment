...
* @example To build a custom augmenter
// require libs
...
class CustomAugmenter extends AbstractAugmenter {
	buildParams({width, height, channels}) {
		...
		// here you should return an hasard object that will be resolved
		// each time the augmenter ran
		return h.object({
			bool: h.boolean(0.5)
		});
	}
	...
	augmentImage({image}, params) {
		// here params.bool is resolved
		// so params.bool is a boolean

		const shouldBeChanged = params.bool;
		if(shouldBeChanged){
			// make your image transformation
			return image.add(1)
		} else {
			return image;
		}
	...
	run(runOpts) {
		...
		if (typeof (this.buildAllImagesHasard) === 'function') {
			...
			for (let i = 0; i < nImages; i++) {
				...
				const params = this.buildHasard(Object.assign({}, metadatas[i], params1, {boxes: params1 && params1.boxes && params1.boxes[i]}));
