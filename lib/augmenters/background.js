...
class BackgroundAugmenter extends AbstractAugmenter {
	...
	buildAllImagesHasard({metadatas, boxes}) {
		...
		return h.object({
			background: this.generator.build(Object.assign({}, metadatas[0], {nImages: metadatas.length, boxes}))
		}, {
			contextName
		});
	}

	augment({images, metadatas, boxes}, {background}) {
		...
		return {
			...
			boxes: boxes.map((b, index) => {
				return b.concat((background.boxes && background.boxes[index]) || []);
			})
		};
	}
}
