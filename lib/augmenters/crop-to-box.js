...
/**
* Crop the image considering input's "boxes" parameter and with
* a variation given by sigma
*
...
* @param {NumberArgument} [opts.minWidth = 0] if the box's width is smaller than minWidth, then use minWidth as width, centered in the center of the box (or as close to)
* @param {NumberArgument} [opts.minHeight = 0] if the box's height is smaller than minHeight, then use minHeight as height, centered in the center of the box (or as close to)
...
class CropToBoxAugmenter extends CropAugmenter {
	constructor(opts, ia) {
		...
		super(o, ia);
		const {sigma, mean = 0, boxIndex, cornersVariation, minWidth, minHeight} = o;
	...
	getRect({width, height, params}) {
		...
		if(width < params.minWidth){
			throw(new Error(`Cannot crop Box with minWidth (${minWidth}) and width (${width})`))
		}
		if(height < params.minHeight){
			throw(new Error(`Cannot crop Box with minHeight (${minHeight}) and height (${height})`))
		}		
		const x = Math.round(
			Math.min(
				Math.max(0, box[0] - (cornersVariation[0][0] * box[2])),
				width - params.minWidth
			)
		);
		const y = Math.round(
			Math.min(
				Math.max(0, box[1] - (cornersVariation[0][1] * box[3])),
				height - params.minHeight
			)
		);
		const w = Math.round(
			Math.max(
				Math.min(width - x, box[2] + ((cornersVariation[0][0] + cornersVariation[1][0]) * box[2])),
				params.minWidth
			)
		);
		const h = Math.round(
			Math.max(
				Math.min(height - y, box[3] + ((cornersVariation[0][1] + cornersVariation[1][1]) * box[3])),
				params.minHeight
			)
		);

		if (isNaN(x) || isNaN(y) || isNaN(w) || isNaN(h)) {
			throw (new TypeError('Cannot use crop-to-box augmenters with NaN box'));
		}
