...
class CropAugmenter extends AbstractAugmenter {
	...
	checkParams({size}) {
		size.forEach(p => {
			if (p < 0) {
				throw (new Error('only positive value allowed in crop for percent'));
			}
		});
	}
	...
	augmentImage({image, width, height}, params) {
		...
		if (rect.w <= 0 || rect.h <= 0) {
			throw (new Error(`Cannot crop by ${JSON.stringify(params)}`));
		}

		const res = this.backend.crop(image, rect);
		return res;
	}
