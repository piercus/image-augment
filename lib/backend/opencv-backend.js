...
class OpenCVBackend {
	...
	overlay({foregrounds, backgrounds, width, height, channels}) {
		if (channels === 3) {
			return foregrounds;
		}

		if (channels === 4) {
			return pluralOperator((foreground, background) => {
				return this.overlayOne({foreground, background, width, height, channels});
