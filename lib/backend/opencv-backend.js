...
class OpenCVBackend {
	...
	pointFromMat(mat) {
		if (mat.rows === 3) {
			return new this._cv.Point(mat.at(0, 0), mat.at(1, 0), mat.at(2, 0));
		}
		...
		throw (new Error(`Cannot build point from Mat with ${mat.rows} rows`));
	}

	pointToArray(point) {
		if (typeof (point.z) === 'number') {
			return [point.at(0), point.at(1), point.at(2)];
		}
	...
	resize(image, width, height) {
		if (typeof (width) !== 'number' || typeof (height) !== 'number') {
			throw (new TypeError('Resize only with numbers'));
		}
	...
	addNoiseOne(img, noiseImg) {
		...
		let res;
		if (img.channels === 4) {
			const [b, g, r, a] = resImg.splitChannels();
			res = new this._cv.Mat([b, g, r, img.splitChannels()[3]]);
		} else {
			res = resImg;
		}
